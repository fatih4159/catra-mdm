...
import static com.agx.catra.control.GainControlService.getComponentName;
...
public class OwnPermissionsWorker extends Worker {
    ...
    public Result doWork() {
        ...
        Constants.USED_PERMISSIONS.forEach(v ->{
            ...
            try{
                isGranted = MainActivity.getDevicePolicyManager().setPermissionGrantState(getComponentName(context), context.getPackageName(), v, DevicePolicyManager.PERMISSION_GRANT_STATE_GRANTED);
            }catch (Exception e){
                e.printStackTrace();
            }
